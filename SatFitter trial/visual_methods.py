...
def results_plot(data,grid,image,params,bg,mu=[],FWHM=[],fitting_radius=4,bg_method='hist'):
	gaussians = gaussianMult(grid,*params) + bg
	...
	if mu == []:
		X,Y = grid
		peaks = len(params)//6
		mu_x = X[np.isnan(data)].mean()
		mu_y = Y[np.isnan(data)].mean()
		mu = np.array(peaks*[[mu_x,mu_y]],float)
        
	if FWHM == []:
		peaks = len(params)//6
		FWHM = np.ones([peaks,2],float)*18.2
        
	fig,[[ax0,ax1],[ax2,ax3]] = plt.subplots(2,2,figsize=(10,10))

	im0 = ax0.imshow(data,origin='lower',extent=[xl,xr,yb,yt],norm=log_norm)
	ax0.set_title('Original image (log scale)')
	...
	ax1.set_title('Fitted image (log scale)')
